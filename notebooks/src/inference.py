...
def model_fn(model_dir):
    ...
    return {"config": config, "device": device, "model": model, "tokenizer": tokenizer}
...
def predict_fn(input_data: dict, model: dict):
    ...
    """
    config, device, trained_model, tokenizer = itemgetter("config", "device", "model", "tokenizer")(
        model
    )
    ...
    with torch.no_grad():
        ...
        # TODO: Support batching if a large number of examples were generated
        ...
        token_page_nums = output_page_nums[..., np.newaxis].repeat(
            output_block_ids.shape[1], axis=1
        )
        flat_page_nums = token_page_nums.flatten()
        flat_probs = probs.reshape(-1, trained_model.num_labels)

        # Filter out tokens that don't correspond to actual Textract 'blocks' (control, pad, etc):
        ...
        avg_probs = np.zeros(
            (len(id_lookup), *filtered_probs.shape[1:]), dtype=filtered_probs.dtype
        )
        ...
        # Use the trp_doc to annotate each WORD block with its classification (which will also
        # update doc_json in place):
